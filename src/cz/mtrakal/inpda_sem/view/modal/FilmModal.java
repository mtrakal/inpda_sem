 ...
 * 
 */
public class FilmModal extends VerticalLayout {

	/**
	 * 
	 */
	private static final long serialVersionUID = 5023805172912344645L;
	FilmotekaApplication app = ApplicationHolder.getApplication();
	Window subwindow;
	Boolean novyPrvek = true;
	Film prvek;

	public FilmModal(String popisek, String ikona) {
		this(popisek, ikona, null);
	}

	public FilmModal(String popisek, String ikona, Item prvek) {
		this(popisek, ikona, prvek, null);
	}

	public FilmModal(String popisekTlacitka, String ikona, Item prvek, String tooltip) {
		if (prvek == null) {
			// nový prvek do DB
			this.prvek = new Film();
		} else {
			// Editace prvku
			novyPrvek = false;
			this.prvek = new Film((Integer) prvek.getItemProperty("Film ID").getValue(), (String) prvek.getItemProperty("Název filmu CZ")
					.getValue(), (String) prvek.getItemProperty("Název filmu EN").getValue(), ((ExternalResource) ((Link) prvek
					.getItemProperty("Odkaz na CSFD").getValue()).getResource()).getURL(), (Integer) prvek.getItemProperty("Rok vydání")
					.getValue(), (Integer) prvek.getItemProperty("Délka").getValue(), (String) prvek.getItemProperty("Popis").getValue());
		}
		BeanItem<Film> pojoItem = new BeanItem<Film>(this.prvek);
		subwindow = new Window((novyPrvek ? "Přidání" : "Editace") + " filmu");
		...
		form.setVisibleItemProperties(Arrays.asList(new String[] { "filmId", "nazevCz", "nazevEn", "linkCsfd", "rokVydani", "delka",
				"popis" }));
