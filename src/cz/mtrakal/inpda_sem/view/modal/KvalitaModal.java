 ...
 * 
 */
public class KvalitaModal extends VerticalLayout {
	...
	public KvalitaModal(String popisek, String ikona) {
		this(popisek, ikona, null);
	}

	public KvalitaModal(String popisek, String ikona, Item prvek) {
		this(popisek, ikona, prvek, null);
	}

	public KvalitaModal(String popisekTlacitka, String ikona, Item prvek, String tooltip) {
		if (prvek == null) {
			// nový prvek do DB
			this.prvek = new Kvalita();
		} else {
			// Editace prvku
			novyPrvek = false;
			this.prvek = new Kvalita((Integer) prvek.getItemProperty("Kvalita ID").getValue(), (String) prvek.getItemProperty("Kvalita")
					.getValue());
		}
		BeanItem<Kvalita> pojoItem = new BeanItem<Kvalita>(this.prvek);
		...
		form.setVisibleItemProperties(Arrays.asList(new String[] { "kvalitaId", "kvalita" }));
	...
	private void showPojoState() {
		...
		n.setDescription("ID: " + prvek.getKvalitaId() + "<br/>Kvalita: " + prvek.getKvalita());
	...
	private class ValidateFieldFactory extends DefaultFieldFactory {
		...
		public Field createField(Item item, Object propertyId, Component uiContext) {
			...
			if ("kvalitaId".equals(propertyId)) {
			...
			} else if ("kvalita".equals(propertyId)) {
				TextField tf = (TextField) f;
				tf.setNullRepresentation("");
				tf.setRequired(true);
				tf.setRequiredError("Vyplň kvalitu");
				// tf.setNullSettingAllowed(true);
				tf.setWidth("12em");
			}
